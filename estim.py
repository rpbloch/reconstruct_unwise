...
class estimator(object):
    ...
    def set_theory_Cls(self, add_ksz = False, add_ml = True, add_lensing =True ,use_cleaned = False, frequency = None, get_haar = False):
        ...
        #self.Cls['taud-lss'] = np.ones((6145,8,1))*1e-14
        ...
        from scipy.interpolate import interp1d    
        #ells = np.append(np.array(lines[0].decode('utf-8').split(' ')).astype('float'), 6145)
        #clgg = np.append(np.array(lines[1].decode('utf-8').split(' ')).astype('float'), 9.2e-8)

        ells = np.array(lines[0].decode('utf-8').split(' ')).astype('float')
        clgg = np.array(lines[1].decode('utf-8').split(' ')).astype('float')

        lssspec = interp1d(ells,clgg, bounds_error=False,fill_value='extrapolate')(np.arange(6145))[:,np.newaxis,np.newaxis]
        ...
        self.Cls['pCMB-pCMB'] = healpy.anafast(Tmap)[:,np.newaxis,np.newaxis]
