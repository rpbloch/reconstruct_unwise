    ...
    def get_pks(self,tag1,tag2,fq1,fq2):
        ...
        if B1 and B2:
            ...
            if c.retag(tag1) in ['e','g','m','tSZ','CIB'] and c.retag(tag2) in ['e','g','m','tSZ','CIB'] :
                ...
                #Pfull_sampled = self.hmod.csm.camb_Pk_nonlin(self.hmod.k,self.hmod.z)
                #Pfull_sampled *= (0.8+1.2*self.conf.zs_hm)[:,np.newaxis]  # UNWISE
                #Pfull_sampled *= (0.8+1.2*self.conf.zs_hm)[:,np.newaxis]  # UNWISE
                #Pfull_sampled += Pmm_lin_sampled
